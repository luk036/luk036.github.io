...
- **Tech Stack**: remark.js, KaTeX, Mermaid, SVGO
...
```bash
# Build topic index.html files and sitemap.xml from README.md
npm run build

# Check internal links in HTML/Markdown
npm run check-links

# Optimize SVGs
npm run svgo

# Lint / auto-fix Markdown (rumdl, configured in rumdl.toml)
npm run lint:md
npm run fmt:md
```
...
4. Run `npm run build` to regenerate the topic `index.html` and `sitemap.xml`
5. Convert to HTML (if needed - some are pre-built)

### Editing Existing Content

1. Edit the source Markdown file
2. Run `npm run build` if the topic has a `README.md`/`index.html` pair
...
- **SVGO** (SVG optimization, devDependency)
- **rumdl** (Markdown linting/formatting; config in `rumdl.toml`, run via `npm run lint:md`)
- **Pre-commit hooks**: Black, isort, flake8 (see `.pre-commit-config.yaml`)
...
1. **Don't hand-edit generated files** - `<topic>/index.html` and `sitemap.xml` are generated by `scripts/build.js`; edit `<topic>/README.md` instead. Hand-maintained `*-remark.html` decks are edited directly.
...
6. **Keep generated files fresh** - `index.html`/`sitemap.xml` are committed, and CI verifies they match `npm run build` output
