...
<pre>
graph TD
  VOS[Voltage Overscaling] --> Nondet[Nondeterministic<br/>Errors]
...
<pre>
graph TD
  Metrics[Evaluation<br/>Metrics] --> Error[Error<br/>Characteristics]
  Metrics --> Circuit[Circuit<br/>Measurements]
  Error --> ER[Error Rate]
  Error --> MED[Mean Error Distance]
  Error --> MRED[Mean Relative<br/>Error Distance]
  Error --> NMSE[Normalized MSE]
  Circuit --> Area[Area]
  Circuit --> Power[Power]
  Circuit --> Delay[Delay]
...
### Approximate Circuit Taxonomy

.mermaid[

<pre>
graph TD
  AxC[Approximate<br/>Circuits] --> Adders[Adders]
  AxC --> Multipliers[Multipliers]
  AxC --> Dividers[Dividers]
  AxC --> MAC[MAC Units]
  AxC --> CORDIC[CORDIC]
