...
<pre>
graph LR
    A("y = 5x") --> B("Binary: 5 = 0101")
    B --> C("x &lt;&lt; 2 (shift left 2)")
    B --> D("x (original)")
    C --> E("➕ Addition")
    D --> E
    E --> F("y = (x &lt;&lt; 2) + x")
...
<pre>
graph TD
    A("Filter Coefficients") --> B("Constant Values")
    B --> C("Bit Pattern Analysis")
...
<pre>
graph TD
    subgraph "CSE Approaches"
        H("Horizontal CSE")
        V("Vertical CSE")
        G("Graph-Based Methods")
    end
    H --> S("Shared Subexpressions")
    V --> S
    G --> S
