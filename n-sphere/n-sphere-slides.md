...
-   Denote $\mathrm{vd}(k,b)$ as a Van der Corput sequence of $k$ points, where $b$ is the base of a prime number.

-   MATLAB source code is available at [here](http://www.mathworks.com/matlabcentral/fileexchange/15354-generate-a-van-der-corput-sequence )
...
-   Deterministic point sets
    ...
    -   Optimal grid point sets for $S^3$, SO(3) \[Lubotzky, Phillips, Sarnak 86\] \[Mitchell 07\]
...
-   $\psi = 2\pi\cdot\mathrm{vd}(k,b_2)$ % map to $[0,2\pi]$ for SO(3), or

-   $\psi = 4\pi\cdot\mathrm{vd}(k,b_2)$ % map to $[0,4\pi]$ for $S^3$
...
-   $p_0 = [\cos\theta_1, \sin\theta_1]$ where $\theta_1 = 2\pi\cdot\mathrm{vd}(k,b_1)$
