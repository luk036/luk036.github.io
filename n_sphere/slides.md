...
-   Can be generated by mapping the van der Corput sequence to $[0, 2\pi]$:
  -   ${\color{teal}\varphi} = 2\pi\cdot{\color{darkgreen}\mathrm{vdc}}(k,b_1)$ % map to $[0,2\pi]$
  -   $[{\color{green}x}, {\color{blue}y}] = [\cos{\color{teal}\varphi}, \sin{\color{teal}\varphi}]$
...
-   The Halton sequence is constructed by combining two van der Corput sequences, which are generated using different prime numbers as the base.
    ...
    $$[{\color{green}x}, {\color{blue}y}] = [{\color{darkgreen}\mathrm{vdc}}(k,b_1), {\color{darkgreen}\mathrm{vdc}}(k,b_2)]$$
...
-   The concept of the Halton sequence can be extended to higher dimensions through the combination of multiple one-dimensional sequences, each generated using a distinct prime number as the base.
    ...
    $$[{\color{green}x_1}, {\color{green}x_2}, \dots, {\color{green}x_n}] = [{\color{darkgreen}\mathrm{vdc}}(k,b_1), {\color{darkgreen}\mathrm{vdc}}(k,b_2), \dots, {\color{darkgreen}\mathrm{vdc}}(k,b_n)]$$
...
-   Cylindrical mapping formula:
  -   ${\color{firebrick}z} = 2\cdot{\color{darkgreen}\mathrm{vdc}}(k,b_2) - 1$ % map to $[-1,1]$
  -   ${\color{teal}\varphi} = 2\pi\cdot{\color{darkgreen}\mathrm{vdc}}(k,b_1)$ % map to $[0,2\pi]$
  -   ${\color{olive}r} = \sqrt{1 - {\color{firebrick}z}^2}$
  -   $[{\color{green}x}, {\color{blue}y}, {\color{firebrick}z}] = [{\color{olive}r}\cos{\color{teal}\varphi}, {\color{olive}r}\sin{\color{teal}\varphi}, {\color{firebrick}z}]$
...
-   Hopf coordinates (cf. [@yershova2010generating])
  -   ${\color{green}x_1} = \cos({\color{steelblue}\theta}/2) \cos({\color{purple}\psi}/2)$
  -   ${\color{green}x_2} = \cos({\color{steelblue}\theta}/2) \sin({\color{purple}\psi}/2)$
  -   ${\color{green}x_3} = \sin({\color{steelblue}\theta}/2) \cos({\color{teal}\varphi} + {\color{purple}\psi}/2)$
  -   ${\color{green}x_4} = \sin({\color{steelblue}\theta}/2) \sin({\color{teal}\varphi} + {\color{purple}\psi}/2)$
...
-   ${\color{teal}\varphi} = 2\pi\cdot{\color{darkgreen}\mathrm{vdc}}(k,b_1)$ % map to $[0,2\pi]$
  -   ${\color{purple}\psi} = 2\pi\cdot{\color{darkgreen}\mathrm{vdc}}(k,b_2)$ % map to $[0,2\pi]$ for SO(3), or
  -   ${\color{purple}\psi} = 4\pi\cdot{\color{darkgreen}\mathrm{vdc}}(k,b_2)$ % map to $[0,4\pi]$ for $S^3$
  -   ${\color{firebrick}z} = 2\cdot{\color{darkgreen}\mathrm{vdc}}(k,b_3) - 1$ % map to $[-1,1]$
  -   ${\color{steelblue}\theta} = \cos^{-1}{\color{firebrick}z}$
...
-   Polar coordinates:
  ...
  -   ${\color{green}x} = {\color{olive}r} \cdot \cos{\color{steelblue}\theta}$
...
### How to generate the point set
  ...
  -   ${\color{steelblue}\theta} = 2\pi\cdot{\color{darkgreen}\mathrm{vdc}}(k,b_1)$ % map to $[0,2\pi]$
  -   ${\color{olive}r} = \sqrt{{\color{darkgreen}\mathrm{vdc}}(k,b_2)}$
  -   $[{\color{green}x}, {\color{blue}y}] = [{\color{olive}r}\cos{\color{steelblue}\theta}, {\color{olive}r}\sin{\color{steelblue}\theta}]$
...
-   Polar coordinates:
  ...
  -   ${\color{green}x_0} = \cos{\color{steelblue}\theta_3}$
  -   ${\color{green}x_1} = \sin{\color{steelblue}\theta_3} \cos{\color{steelblue}\theta_2}$
  -   ${\color{green}x_2} = \sin{\color{steelblue}\theta_3} \sin{\color{steelblue}\theta_2} \cos{\color{steelblue}\theta_1}$
  -   ${\color{green}x_3} = \sin{\color{steelblue}\theta_3} \sin{\color{steelblue}\theta_2} \sin{\color{steelblue}\theta_1}$

-   Spherical surface element:
  $$dA = \sin^{2}({\color{steelblue}\theta_3})\sin({\color{steelblue}\theta_2})\,d{\color{steelblue}\theta_1} \, d{\color{steelblue}\theta_2} d{\color{steelblue}\theta_3}$$
...
-   Polar coordinates:
  ...
  -   ${\color{green}x_0} = \cos{\color{steelblue}\theta_n}$
  -   ${\color{green}x_1} = \sin{\color{steelblue}\theta_n} \cos{\color{steelblue}\theta_{n-1}}$
  -   ${\color{green}x_2} = \sin{\color{steelblue}\theta_n} \sin{\color{steelblue}\theta_{n-1}} \cos{\color{steelblue}\theta_{n-2}}$
  -   ${\color{green}x_3} = \sin{\color{steelblue}\theta_n} \sin{\color{steelblue}\theta_{n-1}} \sin{\color{steelblue}\theta_{n-2}} \cos{\color{steelblue}\theta_{n-3}}$
  -   $\cdots$
  -   ${\color{green}x_{n-1}} = \sin{\color{steelblue}\theta_n} \sin{\color{steelblue}\theta_{n-1}} \sin{\color{steelblue}\theta_{n-2}} \cdots \cos{\color{steelblue}\theta_1}$
  -   ${\color{green}x_n} = \sin{\color{steelblue}\theta_n} \sin{\color{steelblue}\theta_{n-1}} \sin{\color{steelblue}\theta_{n-2}} \cdots \sin{\color{steelblue}\theta_1}$

-   Spherical surface element:
  $$d^nA = \sin^{n-2}({\color{steelblue}\theta_{n-1}})\sin^{n-1}({\color{steelblue}\theta_{n-2}})\cdots \sin({\color{steelblue}\theta_{2}})\,d{\color{steelblue}\theta_1} \, d{\color{steelblue}\theta_2}\cdots d{\color{steelblue}\theta_{n-1}}$$

---

### How to Generate the Point Set 🔄

  -   $p_0 = [\cos{\color{steelblue}\theta_0}, \sin{\color{steelblue}\theta_0}]$ where ${\color{steelblue}\theta_0} = 2\pi\cdot{\color{darkgreen}\mathrm{vdc}}(k,b_0)$
  -   Let $f_j({\color{steelblue}\theta})$ = $\int\sin^j{\color{steelblue}\theta} \mathrm{d}{\color{steelblue}\theta}$, where ${\color{steelblue}\theta}\in (0,\pi)$.
  -   Note 1: The function can be defined recursively as:
    $$
    f_j({\color{steelblue}\theta}) =
    \begin{cases}
      {\color{steelblue}\theta} & \text{if } j = 0 , \\
      -\cos{\color{steelblue}\theta} & \text{if } j = 1 , \\
      (1/n)( -\cos{\color{steelblue}\theta} \sin^{j-1}{\color{steelblue}\theta} + (n-1) f_{j - 2}({\color{steelblue}\theta})) & \text{otherwise}.
    \end{cases}
    $$
...
-   This results in a normalized vector (${\color{green}x_i}/\|{\color{green}x_i}\|$) that is uniformly distributed over the hypersphere $S^n$. (Fishman, G. F. (1996))
