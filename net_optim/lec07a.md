...
#### Definition (Chain ${\color{red}\tau}$)

An edge/node _chain_ ${\color{red}\tau}$ is an $M$/$N$-tuple of scalar
...
The _boundary_ operator ${\color{blue}\partial} = {\color{blue}A}^\mathsf{T}$.

#### Definition (Cycle)

A chain is said to be a _cycle_ if it is in the null-space of the boundary operator, i.e. ${\color{blue}A}^\mathsf{T} {\color{red}\tau} = 0$.
...
-   Let $${\color{red}\tau_i} = \begin{cases}
        1 & \mathrm{if}\ {\color{darkgreen}e_i} \in {\color{red}S}, \\
        0 & \mathrm{otherwise.}
      \end{cases}$$
...
_potential_ ${\color{red}u}$, i.e. ${\color{blue}y}= {\color{green}A} {\color{red}u}$.
...
A path indicator vector ${\color{red}\tau}$ of ${\color{lime}P}$ that $${\color{red}\tau_i} =
    \begin{cases}
...
[total tension ${\color{blue}y}$ on ${\color{lime}P}$] = [total potential on the
boundary of ${\color{lime}P}$].

#### Proof

${\color{blue}y}^\mathsf{T} {\color{red}\tau} = ({\color{green}A} {\color{red}u})^\mathsf{T} {\color{red}\tau} = {\color{red}u}^\mathsf{T}({\color{blue}A}^\mathsf{T} {\color{red}\tau}) = {\color{red}u}^\mathsf{T}({\color{blue}\partial} {\color{lime}P})$.

### Cut

Two node sets ${\color{red}S}$ and ${\color{red}S}'$ (the complement of ${\color{red}S}$, i.e. ${\color{salmon}V} - {\color{red}S}$).
A cut ${\color{steelblue}Q}$ is an edge set, denoted by $[{\color{red}S}, {\color{red}S}']^-$.
...
\begin{cases}
1 & \text{if } {\color{brown}v_i} \in {\color{red}S} \,, \\
0 & \text{otherwise}\,.
\end{cases}
...
[Total divergence of ${\color{green}x}$ on ${\color{red}S}$] = [total ${\color{green}x}$ across ${\color{steelblue}Q}$].
...
-   Find a flow ${\color{green}x}$ such that: $$\begin{array}{ll}
      ...
      {\color{blue}A}^\mathsf{T} {\color{green}x} = {\color{firebrick}b}, {\color{firebrick}b}(V) = 0.
      \end{array}
$$
...
-   ${\color{green}A}$: Stoichiometric matrix $S$
...
The problem has a feasible solution if and only if ${\color{firebrick}b}({\color{red}S}) \leq {\color{green}c^+}({\color{steelblue}Q})$ for all cuts ${\color{steelblue}Q} = [{\color{red}S},{\color{red}S}']$ where ${\color{green}c^+}({\color{steelblue}Q})$ = upper capacity [1, p. 56].
...
-   ${\color{firebrick}b}({\color{red}S}) \leq {\color{green}c^+}({\color{steelblue}Q})$
...
Let ${\color{red}\tau}$ be a path indicator vector (oriented) of ${\color{lime}P}$. Then

-   ${\color{blue}d^-} \leq {\color{blue}y} \leq {\color{blue}d^+}$
-   ${\color{red}\tau}^\mathsf{T} {\color{blue}y} \leq {\color{blue}d^+}({\color{lime}P})$
-   ${\color{red}\tau}^\mathsf{T} ({\color{green}A} \cdot {\color{red}u}) \leq {\color{blue}d^+}({\color{lime}P})$
-   $({\color{blue}A}^\mathsf{T} {\color{red}\tau})^\mathsf{T} {\color{red}u} \leq {\color{blue}d^+}({\color{lime}P})$
-   $({\color{blue}\partial} {\color{lime}P})^\mathsf{T} {\color{red}u} \leq {\color{blue}d^+}({\color{lime}P})$
...
-   ${\color{blue}d^+}$ could be $\infty$ or zero, etc.

-   ${\color{blue}d^-}$ could be $-\infty$ or zero, etc.

-   ${\color{green}c^+}$ could be $\infty$ or zero, etc.
...
-   By splitting every edge into two, the feasibility flow problem can reduce to an elementary one:
  ...
  -   Find a flow ${\color{green}x}$ such that
    ...
    where ${\color{green}A_1}$ is the incident matrix of the modified network.
...
-   By adding a reverse edge for every edge, the feasibility potential problem can reduce to an elementary one:
  ...
  -   Find a potential ${\color{red}u}$ such that
    ...
    $$
    \begin{array}{ll}
      {\color{blue}y}_2 \leq {\color{blue}d}, \\
      {\color{green}A_2} {\color{red}u} = {\color{blue}y}_2
    \end{array}
    $$
...
-   Goal: intentionally assign an arrival time ${\color{red}u_i}$ to each register
  so that the setup and hold time constraints are satisfied.
-   Note: the clock skew ${\color{blue}s}_{ij} = {\color{red}u}_i - {\color{red}u}_j$
...
-   Traditional problem formulation: Find $p$ and ${\color{red}u}$ such that
  ...
  \begin{array}{ll}
    ...
    {\color{green}A} {\color{red}u} = {\color{blue}y}, p \geq 0
  \end{array}
  $$
...
-   Elementary Potential Problem:
  $$
  \begin{array}{ll}
    \text{max} & {\color{firebrick}b}^\mathsf{T} {\color{red}u} - ({\color{green}c}^\mathsf{T} {\color{blue}y} + q) \\
    \text{s. t.} & {\color{blue}y} \leq {\color{blue}d}, \\
    & {\color{green}A} {\color{red}u} = {\color{blue}y}
  \end{array}
  $$
...
-   Since
  ${\color{firebrick}b}^\mathsf{T} {\color{red}u}$ =
  $({\color{blue}A}^\mathsf{T} {\color{green}x})^\mathsf{T} {\color{red}u} = {\color{green}x}^\mathsf{T} {\color{green}A} {\color{red}u} = {\color{green}x}^\mathsf{T} {\color{blue}y},$

## $[\min]-[\max] = ({\color{blue}d}^\mathsf{T} {\color{green}x} + p) - ({\color{firebrick}b}^\mathsf{T} {\color{red}u} - [{\color{green}c}^\mathsf{T} {\color{blue}y} + q])$
...
-   Algorithm idea: descent method: given a feasible
  ...
  ${\color{green}x}_1 = {\color{green}x}_0 + \alpha {\color{green}p}$, where $\alpha$
  is positive.
...
-   Let ${\color{green}x}_1 = {\color{green}x}_0 + \alpha {\color{green}p}$, then we
  have: $$\begin{array}{lll}
      \text{min} & {\color{blue}d}^\mathsf{T} {\color{green}x}_0 + \alpha {\color{blue}d}^\mathsf{T} {\color{green}p} & \Rightarrow {\color{blue}d}^\mathsf{T} {\color{green}p} < 0 \\
      \text{s. t.} & -{\color{green}x}_0 \leq \alpha {\color{green}p} \leq {\color{green}c}-{\color{green}x}_0 & \Rightarrow \text{residual graph} \\
...
-   **while** a negative cycle ${\color{green}p}$ found in $G({\color{green}x})$,
  ...
  3. **If** $\alpha = 0$, **break**;
  4. ${\color{green}x} := {\color{green}x} + \alpha {\color{green}p}$;
...
-   Problem Formulation: $$\begin{array}{ll}
      ...
      \text{s. t.} & {\color{blue}y} \leq {\color{blue}d}, \\
      & {\color{green}A} {\color{red}u} = {\color{blue}y}
    \end{array}$$ where ${\color{green}c}$ is assumed to be non-negative.

-   Algorithm: given an initial feasible ${\color{red}u_0}$, find a better solution ${\color{red}u_1} = {\color{red}u}_0 + \beta {\color{firebrick}q}$, where $\beta$ is positive:
  $$
  \begin{array}{lll}
    \text{min} & {\color{green}c}^\mathsf{T} {\color{blue}y}_0 + {\color{green}c}^\mathsf{T} {\color{blue}y} & \Rightarrow {\color{green}c}^\mathsf{T} {\color{blue}y} < 0 \\
    \text{s. t.} & {\color{blue}y} \leq {\color{blue}d} - {\color{green}A} {\color{red}u}_0 & \Rightarrow \text{residual graph} \\
...
-   **while** a negative cut ${\color{firebrick}q}$ found in $G({\color{red}u})$,
  1. Choose a step size $\beta$;
  2. **If** $\beta$ is unbounded, **return** UNBOUNDED;
  3. **If** $\beta = 0$, **break**;
  4. ${\color{red}u} := {\color{red}u} + \beta {\color{firebrick}q}$;
  5. Update corresponding data structures
-   **return** OPTIMAL
