...
`);return r===-1?0:Hu(e.slice(r+1).match(/^[\t ]*/)[0],t)}var Ku=gp;function Ur(e,t,r){let{node:n}=e;if(n.type==="TemplateLiteral"&&Bp(e)){let m=hp(e,r,t);if(m)return m}let u="expressions";n.type==="TSTemplateLiteralType"&&(u="types");let i=[],a=e.map(t,u),o=Eu(n);o&&(a=a.map(m=>Gr(m,{...r,printWidth:Number.POSITIVE_INFINITY}).formatted)),i.push(ke,"`");let p=0;return e.each(({index:m,node:y})=>{if(i.push(t()),y.tail)return;let{tabWidth:c}=r,f=y.value.raw,l=f.includes(`
`)?Ku(f,c):p;p=l;let h=a[m];if(!o){let S=n[u][m],B=Te(r.originalText,I(y),j(n.quasis[m+1]));if(!B){let O=Gr(h,{...r,printWidth:Number.POSITIVE_INFINITY}).formatted;O.includes(`
`)?B=!0:h=O}B&&(A(S)||q(S)||S.type==="ConditionalExpression"||S.type==="SequenceExpression"||Ae(S)||me(S))&&(h=[E([F,h]),F])}let g=l===0&&f.endsWith(`
`)?ge(Number.NEGATIVE_INFINITY,h):Iu(h,l,c);i.push(D(["${",g,ke,"}"]))},"quasis"),i.push("`"),i}function zu(e){let t=e("quasi");return st(t.label&&{tagged:!0,...t.label},[e("tag"),e("typeParameters"),ke,t])}function hp(e,t,r){let{node:n}=e,s=n.quasis[0].value.raw.trim().split(/\s*\|\s*/);if(s.length>1||s.some(u=>u.length>0)){t.__inJestEach=!0;let u=e.map(r,"expressions");t.__inJestEach=!1;let i=[],a=u.map(c=>"${"+Gr(c,{...t,printWidth:Number.POSITIVE_INFINITY,endOfLine:"lf"}).formatted+"}"),o=[{hasLineBreak:!1,cells:[]}];for(let c=1;c<n.quasis.length;c++){let f=v(!1,o,-1),l=a[c-1];f.cells.push(l),l.includes(`
