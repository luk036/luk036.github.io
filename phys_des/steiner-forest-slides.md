---
title: Steiner Forest via Union-Find and Primal-Dual
author: physdes-py Implementation
date: 2026
theme: default
aspectratio: 169
---
...
3. **Union-Find Data Structure** (5 min)
4. **PD-SF Algorithm** - The Primal-Dual Approach (10 min)
5. **Implementation Deep Dive** - physdes-py Code (7 min)
6. **Proof Sketch: Why ρ=2** (5 min)
7. **Experimental Validation** (3 min)
8. **Conclusion & Future Directions** (2 min)
...
for row in range(height):
    for col in range(width):
        ...
        if row + 1 < height:
            edges.append((node, node + width, 1.0))  # Vertical
```
...
| Return | Description |
|--------|-------------|
| `F_pruned` | List of edges $(u, v, cost)$ in final forest |
| `total_cost` | Sum of all edge costs |
| `sources` | Set of source node indices |
| `terminals` | Set of terminal node indices |
| `steiner_nodes` | Non-terminal nodes used in forest |
...
$$
\leq \sum_t \Delta_t \cdot 2|A_t| = 2 \sum_t \Delta_t |A_t|
$$
...
3. **Theoretical foundation** for modern network design
