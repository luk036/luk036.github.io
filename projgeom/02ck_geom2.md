...
- In hyperbolic geometry, the projection of the fundamental conic to
  the 2D plane is a unit circle, which is called _null circle_. The
...
- Theorem: Let $\tau$ be an involution. Then
  ...
  2.  there is a point $o$ with $\tau(l) = l$ for every line $l$
      incident with $o$.
- We call the line $m$ a _mirror_ and the point $o$ the _center_ of
  the involution.
...
- Then one set of midpoints $m^1_{pq}$_, $m^1_{qr}$_, $m^1_{pr}$ are
  collinear.
